...
Add the **Forecast overlay** visualization to a panel, pick a data source, and choose a model. Mixed metric + Forecast datasource queries are supported on the overlay (Forecast frames are not fitted or plotted there). After a snapshot is saved, the nested **Forecast** datasource can emit forecast / interval series for Grafana alerting by `refId`. Overlay options **New alert rule** opens Grafana’s alerting editor with the panel queries. Grafana’s panel Alert tab is on Time series, not this overlay. The backend caps train size and concurrent Fit / ForecastRange work (413/429); the overlay limits in-flight loads per panel. The Docker Grafana environment is the sibling `timeseries-grafana-sandbox` (provisioned: minute-of-week overlay and Metrics vs baselines).
