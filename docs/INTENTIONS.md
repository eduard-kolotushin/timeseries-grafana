...
- **`trainSource` in the fit request**: `POST /forecast` gains an optional `trainSource {datasourceUid, queries, from, to, seriesName, relative, lookbackMs}` carrying exactly what the browser sent to the datasource, plus how its window was resolved: a lookback window is `relative: true` with `lookbackMs`, so a cron retrain re-resolves `[now − lookbackMs, now]` at claim time instead of refetching one frozen range forever; an explicit picker is `relative: false` and is replayed verbatim. A successful fit upserts `forecast.retrain` for that `cacheKey` (never resetting an existing row's cron). `trainSource` and its window fields are **not** part of the `cacheKey` fingerprint
