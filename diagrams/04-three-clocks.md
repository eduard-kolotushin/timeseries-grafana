...
Ключ снимка берёт **сырые** строки `trainRange.from` / `to` (и устаревший `lookback`), не unix ms. Auto вроде `now-21d` не инвалидирует снимок когда ползёт `now`. Модель может отставать от текущего времени до Retrain.
